...
import unitKerjaServices from "@/services/unitKerjaServices";
import NoSSRWrapper from "@/components/NoSSRWrapper";

const RuasAll = await RuasService.getRuasAll(1,5);
const unitkerja = await unitKerjaServices.get();
function MasterData() {
...
<NoSSRWrapper>
    <section>
      ...
      <Modal opened={isView} onClose={()=>{setView(false)}} title="Detail Ruas">
        ...
        readOnly
        />
      <FileInput label="Foto*" placeholder={fotoUrl.split("/").pop()} icon={<ArrowUpTrayIcon />}  readOnly/>
      <FileInput label="Dokumen *" placeholder={dokumentUrl.split("/").pop()} icon={<ArrowUpTrayIcon />} readOnly/>
      <TextInput
        label="Panjang (km)*"
